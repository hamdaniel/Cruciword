...
LETTER_TO_INDEX = {letter: i for i, letter in enumerate(HEBREW_ALPHABET)}
MIN_LENGTH = 2
MAX_LENGTH = 15
...
class BitArray:
	...
	def first_one(self) -> int | None:
		for i in range(self.logical_size):
			if self._get_raw_bit(i):
				return i
		return None
	...
	def set_all(self, value) -> None:
		...
		for i in range(len(self.data)):
			self.data[i] = 0xFF if value else 0x00

		self._clear_unused_tail_bits()
		self._set_any_bit(1 if value else 0)
	...
	def copy(self) -> "BitArray":
		...
		result.data[:] = self.data  # deep copy of bytes
		return result
